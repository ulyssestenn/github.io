...
GUIDES = M['guides']
LASTMOD = M.get('lastmod', '2026-09-08')
...
APP = {
    ...
    'loan-it': ('Loan It','/loanit/','https://ulix.app/icons/loanit.png','summary'),
...
def parse(text):
    ...
    first_para = plain(re.split(r'\n\s*\n', body)[0])
    deck = plain(meta.get('deck', '')) or first_para
    return title, meta, body, product, deck

def page(g, source):
    title, meta, body, product, deck = parse(source)
    author = meta.get('author', g['author'])
    section = g['section']
    app_name, app_url, image, card = APP[g['app_slug']]
    ...
    related = ''.join(
        f'<a href="../{html.escape(x["slug"])}/">{html.escape(x["title"])}</a>'
        for x in g.get('related', [])
    )
    ...
    schema = {
        ...
        'about':{'@type':'SoftwareApplication','name':app_name,'operatingSystem':'Android','url':'https://ulix.app'+app_url},
...
parsed = []
for g in GUIDES:
    src = (BATCH / (g['slug'] + '.md')).read_text()
    title, meta, body, prod, deck = parse(src)
    author = meta.get('author', g['author'])
    parsed.append((g,title,author,deck,src))
    out = ROOT / 'knowledge' / g['slug'] / 'index.html'
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(page(g,src))

new = list(reversed(parsed))

idxp = ROOT / 'knowledge/guides/index.html'
idx = idxp.read_text()
for g in GUIDES:
    slug = re.escape(g['slug'])
    idx = re.sub(rf'<a class="guide-index-card"[^>]*data-slug="{slug}"[^>]*>.*?</a>\s*', '', idx, flags=re.S)
cards = '\n'.join(
    f'<a class="guide-index-card" data-guide-card data-slug="{g["slug"]}" data-section="{html.escape(g["section"],quote=True)}" href="../{g["slug"]}/"><span>{html.escape(a)} · {html.escape(g["section"])}</span><h2>{html.escape(t)}</h2></a>'
    for g,t,a,d,s in new
) + '\n'
...
jsp = ROOT / 'knowledge/guides/guides.js'
js = jsp.read_text()
by = {}
for g in GUIDES:
    by.setdefault(g['app_slug'], []).append(g['slug'])
for slug, slugs in by.items():
    pat = re.compile(rf"('{re.escape(slug)}': new Set\(\[\n\s*)(.*?)(\n\s*\]\),?)", re.S)
    m = pat.search(js)
    if m:
        cur = m.group(2).rstrip()
        add = [s for s in slugs if f"'{s}'" not in cur]
        if add:
            cur = cur + (',' if cur and not cur.endswith(',') else '') + ','.join(f"'{s}'" for s in add)
            js = js[:m.start(2)] + cur + js[m.end(2):]
    else:
...
hp = ROOT / 'knowledge/index.html'
home = hp.read_text()
pat = re.compile(r'(<section class="knowledge-section" aria-labelledby="recent-guides-heading">.*?<div class="knowledge-grid">\n)(.*?)(\n</div></section>)', re.S)
m = pat.search(home)
old = re.findall(r'<a class="knowledge-card".*?</a>', m.group(2), re.S)
newcards = [
    f'<a class="knowledge-card" href="./{g["slug"]}/"><div class="knowledge-card__meta"><span>{html.escape(a)}</span><span>{html.escape(g["section"])}</span></div><h3>{html.escape(t)}</h3><p>{html.escape(d)}</p></a>'
    for g,t,a,d,s in new
]
seen = set()
allcards = []
...
for slug in dict.fromkeys(g['app_slug'] for g in GUIDES):
    if f'./guides/?app={slug}' not in home:
        ...
        if am:
            home = home[:am.end(2)] + f'<a class="app-link" href="./guides/?app={slug}">{APP[slug][0]}</a>' + home[am.end(2):]
hp.write_text(home)

lp = ROOT / 'llms.txt'
ll = lp.read_text()
for g in GUIDES:
    ll = re.sub(rf'^- \[[^\n]+\]\(https://ulix\.app/knowledge/{re.escape(g["slug"])}/\)\n?', '', ll, flags=re.M)
lm = re.search(r'(\[All Knowledge Guides\]\(https://ulix\.app/knowledge/guides/\).*?\n\n)',ll,re.S)
lines = '\n'.join(f'- [{t}](https://ulix.app/knowledge/{g["slug"]}/)' for g,t,a,d,s in new) + '\n'
ll = ll[:lm.end()] + lines + ll[lm.end():]
lp.write_text(ll)

sp = ROOT / 'sitemap.xml'
sm = sp.read_text()
for loc in ['https://ulix.app/knowledge/','https://ulix.app/knowledge/guides/']:
    sm = re.sub(rf'(<url><loc>{re.escape(loc)}</loc><lastmod>)[^<]+', rf'\g<1>{LASTMOD}', sm)
for g in GUIDES:
    sm = re.sub(rf'\s*<url><loc>https://ulix\.app/knowledge/{re.escape(g["slug"])}/</loc>.*?</url>\s*', '\n', sm, flags=re.S)
entries = '\n'.join(
    f'  <url><loc>https://ulix.app/knowledge/{g["slug"]}/</loc><lastmod>{LASTMOD}</lastmod><changefreq>monthly</changefreq><priority>0.7</priority></url>'
    for g,t,a,d,s in new
) + '\n'
mm = re.search(r'  <url><loc>https://ulix\.app/knowledge/guides/</loc>.*?</url>\n', sm)
sm = sm[:mm.end()] + entries + sm[mm.end():]
sp.write_text(sm)
